...
class WPTelegram_Admin extends WPTelegram_Core_Base {
	...
	public function create_options_pages() {
		...
		$cmb2->add_field(
			array(
				...
				'desc'            => '<br>' . sprintf( /* translators: %s is @ */__( 'Telegram Bot username (without %s).', 'wptelegram' ), '<code>@</code>' ),
				'id'              => 'bot_username',
				'after'           => sprintf( /* translators: %s is Name of the button */ __( 'Use %s to set automatically.', 'wptelegram' ), '<b>' . __( 'Test Token', 'wptelegram' ) . '</b>' ),
				'type'            => 'text_medium',
	 ...
	 * @param array $args The arguments of the below shape.
	 *
	 * {
		...
		if ( ! empty( $args['should_notify'] ) ) {
			...
			if ( ! empty( $invalid_fields ) ) {
				...
				foreach ( (array) $invalid_fields as $field => $status ) {
					...
					if ( 'empty' == $status ) {
						$args['message'] = sprintf( /* translators: field name */ esc_html__( '%s is empty', 'wptelegram' ), $field_name );
					} else {
						$args['message'] = sprintf( /* translators: field name */ esc_html__( 'Invalid %s', 'wptelegram' ), $field_name );
					}
	...
	public function render_plugin_header( $cmb_id, $object_id, $object_type, $cmb2 ) {
		...
		if ( 'options-page' === $object_type && preg_match( $pattern, $object_id ) ) {
			...
			?>
			<h3>
				<?php esc_html_e( 'Want an absolute integration with Telegram?', 'wptelegram' ); ?>&nbsp;
				<a class="button button-primary" href="https://wptelegram.pro" target="_blank"><?php esc_html_e( 'Upgrade to Pro', 'wptelegram' ); ?></a>
			</h3>
			<?php
		}
	}
	...
	public function render_plugin_sidebar( $hookup ) {
		...
		<div class="wptelegram-box wptelegram-column-2">
			<div class="inner">
				<div class="cell">
					<h2><?php echo esc_html( WPTG()->get_plugin_title() ); ?></h2>
				</div>
				<div class="cell">
					<p><?php esc_html_e( 'Integrate your WordPress website perfectly with Telegram.', 'wptelegram' ); ?></p>
				</div>
				<div class="cell">
					<p>
						<?php printf( /* translators: plugin name */ esc_html__( 'Do you like %s?', 'wptelegram' ), esc_html( WPTG()->get_plugin_title() ) ); ?>
						<br>
				...
				<div class="cell">
					<p>
						<?php esc_html_e( 'Need more features?', 'wptelegram' ); ?>
						<br><br>
						<a class="button button-primary" href="https://wptelegram.pro" target="_blank"><?php esc_html_e( 'Upgrade to Pro', 'wptelegram' ); ?></a>
					</p>
				</div>
				<div class="cell">
					<p>
						<?php esc_html_e( 'Need help?', 'wptelegram' ); ?>
						<br>
						<b><?php esc_html_e( 'Get LIVE support on Telegram', 'wptelegram' ); ?></b>
						<br>
				...
				<div class="cell">
					<h3><?php esc_html_e( 'What is NEW?', 'wptelegram' ); ?></h3>
			...
			<div class="footer">
				<p><?php printf( /* translators: plugin name */ esc_html__( 'Enjoy %s', 'wptelegram' ), esc_html( WPTG()->get_plugin_title() ) ); ?> 🙂</p>
			</div>
		</div>
	...
	public static function render_telegram_guide( $field_args, $field ) {
		...
		<div class="cmb-row cmb-type-text cmb2-id-telegram_guide">
			<ol style="list-style-type: decimal;">
				<li>
				<?php
				printf(/* translators: 1 - command name, 2 - Bot */ __( 'Create a Bot by sending %1$s command to %2$s', 'wptelegram' ), '<code>/newbot</code>', '<a href="https://t.me/BotFather"  target="_blank">@BotFather</a>' );
				?>
				</li>
				<li><?php printf( esc_html__( 'After completing the steps %s will provide you the Bot Token.', 'wptelegram' ), '@BotFather' ); ?></li>
	...
	public static function render_after_field( $field_args, $field ) {
		...
		<?php if ( 'bot_token' === $_id ) : ?>
			<p class="as-row">
				<span class="hidden wptelegram-info <?php echo esc_html( $_id ); ?>-test"><?php esc_html_e( 'Test Result:', 'wptelegram' ); ?>&nbsp;</span>
				<span class="hidden wptelegram-info <?php echo esc_html( $_id ); ?>-info"></span>
				<span class="hidden wptelegram-info error <?php echo esc_html( $_id ); ?>-err"><?php esc_html_e( 'Invalid Bot Token', 'wptelegram' ); ?></span>
			</p>

		<?php elseif ( 'bot_username' === $_id ) : ?>
			<p>
				<span class="hidden wptelegram-info error <?php echo esc_html( $_id ); ?>-err"><?php esc_html_e( 'Invalid Bot Username', 'wptelegram' ); ?></span>
			</p>
		<?php endif; ?>
		<?php
	}
	...
	public static function render_after_module( $field_args, $field ) {
		...
		if ( in_array( $id, $active_modules, true ) ) {
		...
		printf( '<span><a %s>%s</a></span>', $atts, esc_html__( 'Settings', 'wptelegram' ) );
